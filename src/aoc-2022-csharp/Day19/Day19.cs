...
public static class Day19
{
        ...
        int oreRobots, int clayRobots, int obsidianRobots, int geodeRobots)
    {
        ...
        var maxOre = new[] { blueprint.OreRobotOreCost, blueprint.ClayRobotOreCost, blueprint.ObsidianRobotOreCost,
        ...
        ore = Math.Min(ore, minutesLeft * maxOre - oreRobots * (minutesLeft - 1));
        clay = Math.Min(clay, minutesLeft * maxClay - clayRobots * (minutesLeft - 1));
        obsidian = Math.Min(obsidian, minutesLeft * maxObsidian - obsidianRobots * (minutesLeft - 1));

        best = Math.Max(best, MaxScore(blueprint, minutesLeft - 1, ore + oreRobots, clay + clayRobots,
            obsidian + obsidianRobots, geodes + geodeRobots, oreRobots, clayRobots, obsidianRobots, geodeRobots));

        if (CanBuildGeodeRobot(blueprint, ore, obsidian))
        {
            best = Math.Max(best, MaxScore(blueprint, minutesLeft - 1, ore + oreRobots - blueprint.GeodeRobotOreCost,
                clay + clayRobots, obsidian + obsidianRobots - blueprint.GeodeRobotObsidianCost, geodes + geodeRobots,
                oreRobots, clayRobots, obsidianRobots, geodeRobots + 1));
        }

        if (obsidianRobots < maxObsidian && CanBuildObsidianRobot(blueprint, ore, clay))
        {
            best = Math.Max(best, MaxScore(blueprint, minutesLeft - 1, ore + oreRobots - blueprint.ObsidianRobotOreCost,
                clay + clayRobots - blueprint.ObsidianRobotClayCost, obsidian + obsidianRobots, geodes + geodeRobots,
                oreRobots, clayRobots, obsidianRobots + 1, geodeRobots));
        }

        if (clayRobots < maxClay && CanBuildClayRobot(blueprint, ore))
        {
            best = Math.Max(best, MaxScore(blueprint, minutesLeft - 1, ore + oreRobots - blueprint.ClayRobotOreCost,
                clay + clayRobots, obsidian + obsidianRobots, geodes + geodeRobots, oreRobots, clayRobots + 1,
                obsidianRobots, geodeRobots));
        }

        if (oreRobots < maxOre && CanBuildOreRobot(blueprint, ore))
        {
            best = Math.Max(best, MaxScore(blueprint, minutesLeft - 1, ore + oreRobots - blueprint.OreRobotOreCost,
                clay + clayRobots, obsidian + obsidianRobots, geodes + geodeRobots, oreRobots + 1, clayRobots,
                obsidianRobots, geodeRobots));
        }
